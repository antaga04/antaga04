...
def accessible_edges(edges):
    """
    Filters out edges whose node is null, which GitHub returns for repositories that
    cannot be resolved (deleted, transferred, or temporarily inaccessible to the token)
    """
...
def graph_repos_stars(count_type, owner_affiliation, cursor=None, add_loc=0, del_loc=0):
    ...
    if request.status_code == 200:
        if count_type == 'repos':
            ...
            return stars_counter(accessible_edges(request.json()['data']['user']['repositories']['edges']))
...
if __name__ == '__main__':
    """
    Adrian Anta (Antaga04), 2022-2025
    """
    print('Calculation times:')
    # define global variable for owner ID and calculate user's creation date
    # e.g {'id': 'MDQ6VXNlcjU3MzMxMTM0'} and 2019-11-03T21:15:07Z for username 'Antaga04'
    user_data, user_time = perf_counter(user_getter, USER_NAME)
    OWNER_ID, acc_date = user_data
    formatter('account data', user_time)
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2003, 11, 4))
